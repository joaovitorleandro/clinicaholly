  ...
  <section id="resultados" class="results" aria-labelledby="results-title">
    <div class="container">
      ...
      <div class="results-gallery">
        <RevealBlock v-for="(result, index) in results" :key="result.label" as="figure" class="result-photo" :class="`result-photo-${index + 1}`">
          ...
          <figcaption><span>{{ result.label }}</span><span class="caption-number">0{{ index + 1 }} <span aria-hidden="true">↗</span></span></figcaption>
        </RevealBlock>
        <div class="gallery-aside" aria-hidden="true"><HollyStar /><span>HOLLY — ESTÉTICA INTEGRADA</span></div>
      </div>
      <div class="results-bottom"><a class="text-link" :href="clinic.instagram" target="_blank" rel="noopener noreferrer">Ver mais resultados no Instagram <span aria-hidden="true">↗</span></a><span class="eyebrow">Cada caso é único</span></div>
    </div>
    <dialog ref="dialog" class="result-dialog" aria-label="Galeria de resultados da Clínica Holly" aria-describedby="result-dialog-note" @click="backdropClick" @close="onClose" @keydown.left.prevent="move(-1)" @keydown.right.prevent="move(1)">
      <div class="dialog-content">
        ...
        <div class="dialog-bottom"><button aria-label="Fotografia anterior" @click="move(-1)">←</button><span aria-live="polite">0{{ selected + 1 }} / 0{{ results.length }}</span><button aria-label="Próxima fotografia" @click="move(1)">→</button></div>
